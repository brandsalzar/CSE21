...
public class Shop {
	...
	private void intro(Scanner input) {
		...
		System.out.print("Enter amount of " + Brie.getName() + " : ");//added
		Brie.setAmount(input.nextInt());                              //added
		System.out.print("Enter amount of " + Swiss.getName() + " : ");//added
		Swiss.setAmount(input.nextInt());                               //added
		// Fill-in Code
	}

	private double calcSubTotal() {
		double subTotal = 0;
		subTotal += (Sharp.getAmount() * Sharp.getPrice())+(Brie.getAmount()*Brie.getPrice())+(Swiss.getAmount()*Swiss.getPrice());// added: (Brie.getAmount()*Brie.getPrice())+(Swiss.getAmount()*Swiss.getPrice()
		// Fill-in Code
	...
	private void itemizedList(){
		...
		if ((amt = Brie.getAmount()) > 0)                                                                  	 	 	 	 	 	   	 	//added
			System.out.println(amt + " lbs of Stormwind "+ Brie.getName() +"  @ $"+ Brie.getPrice() +"  = " + (amt * Brie.getPrice()));//added
		if ((amt = Swiss.getAmount()) > 0)                                    											 	 	 	 	//added
			System.out.println(amt + " lbs of Alterac "+ Swiss.getName() + "  @ $"+ Swiss.getPrice() +"  = " + (amt * Swiss.getPrice()));//added
		// Fill-in Code
	}
